...
if ((__indexOf.call(process.argv, '--profile') >= 0)) {
  ...
  lazyRequire('webkit-devtools-agent', {
    ...
    return console.log("Profiling with process id:", process.pid);
  });
}
  ...
  */
  ...
  function DocPad(instanceConfig, next) {
    ...
    this.errorRunnerInstance = new TaskGroup().run().on('complete', function(err) {
      ...
      if (err && docpad.getDebugging()) {
        ...
        return docpad.log('warn', locale.reportError + '\n' + locale.errorFollows, ((_ref2 = err.stack) != null ? _ref2 : err.message).toString());
      }
    });
  ...
  DocPad.prototype.ready = function(opts, next) {
    ...
    tasks.addTask(function(complete) {
      ...
      return _this.emitSerial('welcome', {
        docpad: docpad
      }, complete);
    });
    tasks.addTask(function(complete) {
  ...
  DocPad.prototype.track = function(name, things, next) {
    var data, helperUrl, reportStatistics, _ref1, _ref2, _ref3,
      _this = this;
    if (things == null) {
      things = {};
    }
    _ref1 = this.getConfig(), reportStatistics = _ref1.reportStatistics, helperUrl = _ref1.helperUrl;
    if (reportStatistics && ((_ref2 = this.userConfig) != null ? _ref2.username : void 0)) {
      data = {};
      data.userId = this.userConfig.username;
      data.event = name;
      data.properties = things;
      if ((_ref3 = this.websitePackageConfig) != null ? _ref3.name : void 0) {
      ...
      this.getTrackRunner().addTask(function(complete) {
        return superAgent.post(helperUrl).type('json').set('Accept', 'application/json').query({
          method: 'analytics',
          action: 'track'
        }).send(data).timeout(30 * 1000).end(_this.checkRequest(complete));
      });
    }
    if (typeof next === "function") {
      next();
  ...
  DocPad.prototype.identify = function(next) {
    var data, helperUrl, now, reportStatistics, things, _ref1, _ref2,
      _this = this;
    _ref1 = this.getConfig(), reportStatistics = _ref1.reportStatistics, helperUrl = _ref1.helperUrl;
    if (reportStatistics && ((_ref2 = this.userConfig) != null ? _ref2.username : void 0)) {
      data = {};
      ...
      things.version = this.getVersion();
      things.nodeVersion = this.getProcessVersion();
      if (this.userConfig.identified !== true) {
        things.created = now.toISOString();
        this.getTrackRunner().addTask(function(complete) {
          return superAgent.post(helperUrl).type('json').set('Accept', 'application/json').query({
            method: 'analytics',
            action: 'identify'
          }).send(data).timeout(30 * 1000).end(_this.checkRequest(function(err) {
            _this.updateUserConfig({
              identified: true
            });
            return complete(err);
          }));
  ...
  DocPad.prototype.init = function(opts, next) {
    var config, docpad, locale, srcPath, _ref1;
    _ref1 = extractOptsAndCallback(opts, next), opts = _ref1[0], next = _ref1[1];
    docpad = this;
    config = this.getConfig();
    srcPath = config.srcPath;
